...
emptyNode : Node
emptyNode =
    { nodeType = Rect rectDef, status = Disabled }
...
textNode : Text -> Node
textNode text =
    { nodeType = Text { text = text }, status = Enabled }


extentIsFill : Which Extent -> Node -> Bool
extentIsFill which node =
    which `extentOf` node |> isFill


extentIsFix : Which Extent -> Node -> Bool
extentIsFix which node =
    which `extentOf` node |> isFix


fixSize : Which Extent -> Node -> Float
fixSize which node =
    which `extentOf` node |> map fixSize' |> withDefault 0.0
...
isFill extent =
    withDefault False <| map isFill' extent
...
isFix extent =
    withDefault False <| map isFix' extent
