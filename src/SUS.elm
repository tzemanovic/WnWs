...
susDefToNode def =
    let
        susOptions : List ( String, ( String, String ) )
        susOptions =
            findShortestUniqueSubstrings def.options

        shortOptions : List String
        shortOptions =
            List.map (\( _, ( short, _ ) ) -> short) susOptions

        mc : SusState
        mc =
            matchCount def.content def.options
...
matchCount c options =
    if String.isEmpty c.string then
        Ambiguous
    else
        case List.filter (String.startsWith c.string) options of
            x :: [] ->
                Match x
...
highlightMatch short content =
    if
        not (String.isEmpty content.string)
            && (String.startsWith content.string short)
    then
        Filled green
    else
...
findShortestUniqueSubstrings :
    List String
    -> List ( String, ( String, String ) )
findShortestUniqueSubstrings os =
    List.map
        (\o ->
            let
                otherOs =
                    List.filter ((/=) o) os
            in
                findShortestUniqueSubstring 1 otherOs o
        )
        os
...
findShortestUniqueSubstring chars os option =
    let
        short =
            String.left chars option

        isUnique =
            List.all (\o -> String.left chars o /= short) os

        afterShort =
            String.right (String.length option - chars) option
    in
        if isUnique then
            ( option, ( short, afterShort ) )
        else
            findShortestUniqueSubstring (chars + 1) os option
